...
from app.engines.hybrid_rag.rag import DOCS_DIR, ROOT, _chunk_markdown
...
def _connect(url: str):
    ...
    # Extension must exist before register_vector binds the type.
...
def available(settings: Optional[Settings] = None) -> bool:
    ...
    try:
        ...
        return True
    except Exception as exc:  # noqa: BLE001 — fallback to keyword
...
def ensure_schema(conn) -> None:
    with conn.cursor() as cur:
        ...
        cur.execute(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS doc_chunks_path_idx
            ...
            for row, vec in zip(to_embed, vectors):
                ...
                cur.execute(
                    ...
                    INSERT INTO doc_chunks
                      (id, path, chunk_index, title, body, content_sha, embedding)
    ...
    *,
    ...
    with _connect(url) as conn:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT path, chunk_index, title, body,
            ...
            for path, idx, title, body, _dist in cur.fetchall():
                out.append(
                    {
                        "score": 0.0,
                        "citation": {
                            "type": "doc",
                            "ref": "{0}#chunk-{1}".format(path, idx),
                            "title": title,
                            "snippet": (body or "")[:240],
                        },
                        "text": body,
